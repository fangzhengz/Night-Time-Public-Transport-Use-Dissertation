...
审计性质：2026-08-22完成只读来源核查；2026-08-25追加一次发布前复核
...
- **主要研究过程覆盖：通过补录。** 初次审计时目标库索引22项研究记录；2026-08-25复核后共有36项，覆盖重要模型发展、敏感性分析、替代背景分析、未完成的“地区流动与公共交通使用偏离”分析、Bus–Rail探索及一项此前仅存在于原工作区的历史LNWC空间图。
...
1. `research_record/STATUS.csv` 已由22项扩展到36项；第36项是在2026-08-25结果入口复核中补入的历史LNWC空间图及其生成脚本。
...
3. 未完成的“地区流动与公共交通使用偏离”分析已统一说明为：输出仅作为历史快照保留，因早于配置修正而不可引用；原内部文件中的早期编号只是历史工作标签。
