...
2. **all-modes(404 站)**:本文件夹新建的流水线,保留全部 NUMBAT
   轨道模式(LU、DLR、Overground、伊丽莎白线、电车),并对同一物理地点但
   跨模式分开记账的站点做了合并(`01b_merge_colocated_stations.py`,例如
   希思罗各航站楼的 Underground 侧与伊丽莎白线侧)。441 个合并后的原始
   站点中有 404 个在夜间时段有活动量(其余 37 个总活动量为0,
...
441 个原始站点中被剔除的 37 个,**全部**是电车(TRM)专属站点,
...
- all-modes(404 站,全部轨道模式)BIC 最优在 **K=5**,
  且这不是网格边界效应——K=5 前后的 BIC 都比它差
  (K4=-1442874,
  K5=-1446369,
  K6=-1445565),是一个真实的内部极值。
...
把 all-modes(404站)K=5 的结果限制回原来的270个 Underground 站点,与
...
## 发现三:新增的134个非 Underground 站点去了哪里
...
- 但原270个 Underground 站点在新聚类中的簇归属部分一致
  (ARI=0.534),说明新增的134站主要是在"补充"聚类结构,
...
- all-modes 的404站里有134个站点是本次检验才第一次纳入特征构建流程,没有
