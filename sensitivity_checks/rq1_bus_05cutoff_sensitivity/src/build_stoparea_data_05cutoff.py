# -*- coding: utf-8 -*-
"""Sensitivity sidecar: identical StopArea/LSOA aggregation logic to
`data_processing/bus_stoparea/src/build_stoparea_data.py`, with only the
input stop-flow parquet and output tree redirected to this sidecar's own
05:00-cutoff preprocessed BUSTO output. Canonical outputs are never read or
written here.

Only two things differ from the canonical script:
  - STOP_FLOW_PARQUET points at
    `rq1_bus_05cutoff_sensitivity/preprocessed_busto_05cutoff/busto_stop_qhr_night.parquet`
    (built with --end-min 1740, i.e. night window 18:00-05:00) instead of the
    canonical 18:00-06:00 output.
  - OUT (and PRE/DATA/AUDIT/REPORT under it) point at this sidecar's own
    `bus_stoparea_05cutoff/outputs/` tree.
Everything else -- StopArea grouping, LSOA assignment, conservation checks --
is unchanged from the canonical script.
"""
...
STOP_FLOW_PARQUET = (
    SIDECAR_ROOT / "preprocessed_busto_05cutoff" / "busto_stop_qhr_night.parquet"
)
LEGACY_POINT_LOOKUP = (
    FYP
...
OUT = SIDECAR_ROOT / "bus_stoparea_05cutoff" / "outputs"
...
def build_stop_crosswalk(
    ...
    flow: pd.DataFrame,
    legacy_lookup: pd.DataFrame | None,
) -> pd.DataFrame:
    ...
    if legacy_lookup is not None:
        stop_crosswalk = stop_crosswalk.merge(
            legacy_lookup[["stopcode", "lsoa"]].rename(
...
def main() -> None:
    ...
    legacy_lookup: pd.DataFrame | None = None
    if LEGACY_POINT_LOOKUP.exists():
        legacy_lookup = pd.read_csv(LEGACY_POINT_LOOKUP, dtype=str)
        legacy_lookup["stopcode"] = legacy_lookup["stopcode"].astype(str)
        if legacy_lookup["stopcode"].duplicated().any():
            raise RuntimeError("Legacy point-LSOA lookup contains duplicate stopcodes.")
    ...
    if legacy_lookup is not None:
        manifest.append(
            manifest_row(
                "optional_legacy_point_lsoa_lookup",
                LEGACY_POINT_LOOKUP,
                len(legacy_lookup),
                "Audit-only input; never used to assign StopArea units.",
            )
        )

    stops = build_stop_crosswalk(points, areas, bus_stops, flow, legacy_lookup)
    ...
    stops["changed_vs_original"] = (
        stops["old_lsoa"].notna()
        & stops["stoparea_lsoa"].notna()
    ...
    summary_rows = [
        ...
        (
            "activity_changed_vs_original",
            float(stops.loc[stops["changed_vs_original"], "total_activity"].sum()),
        ),
        (
            "n_stops_newly_included_vs_original",
            int(stops["newly_included_vs_original"].sum()),
        ),
        (
            "n_stops_newly_excluded_vs_original",
            int(stops["newly_excluded_vs_original"].sum()),
        ),
    ...
    report = [
        "# Bus child-StopArea preprocessing -- 05:00-cutoff sensitivity sidecar",
        "",
        "Identical method to the canonical `data_processing/bus_stoparea` build;",
        "the only difference is the upstream stop-flow input, which was built with",
        "`--end-min 1740` (18:00-05:00 night window) instead of the canonical",
        "`--end-min 1800` (18:00-06:00).",
        "",
        "## Method boundary",
        "",
