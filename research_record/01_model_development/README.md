...
This section keeps those journeys visible. `method_experiments/` contains the alternatives that isolated individual assumptions or tested possible replacements, including temporal resolution, Hellinger/ILR geometry and competing Bus spatial units. `formal_sensitivity_checks/` contains focused checks of geography, K, time cutoffs and the Rail weekend metric. `rail_scope_and_stability/` records the successive Rail populations and K batteries, while `adopted_bus_diagnostics_snapshot/` preserves the fuller diagnostic evidence surrounding the final Bus route.
