...
Selecting K=4 by BIC answered which fitted model scored best, but it did not show whether the same substantive structure would reappear from different initial conditions or at an adjacent K. This check was built to close that gap. It follows the fate of the night-active clusters across seeds and between K=4 and K=5, treating stability as evidence about uncertainty rather than as a second model-selection rule.

The sidecar was run on 2026-08-03. It reads `rq1_bus_stoparea_clustering`'s CLR feature
