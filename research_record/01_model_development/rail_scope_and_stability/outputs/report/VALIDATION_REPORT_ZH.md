...
2. **all-modes(403 站)**:本文件夹新建的流水线,保留全部 NUMBAT
   轨道模式(LU、DLR、Overground、伊丽莎白线、电车),并对同一物理地点但
   跨模式分开记账的站点做了合并(`01b_merge_colocated_stations.py`,例如
   希思罗各航站楼的 Underground 侧与伊丽莎白线侧)。440 个合并后的原始
   站点中有 403 个在夜间时段有活动量(其余 37 个总活动量为0,
...
440 个原始站点中被剔除的 37 个,**全部**是电车(TRM)专属站点,
...
- all-modes(403 站,全部轨道模式)BIC 最优在 **K=5**,
  且这不是网格边界效应——K=5 前后的 BIC 都比它差
  (K4=-1898039,
  K5=-1899714,
  K6=-1898930),是一个真实的内部极值。

**这直接回答了 Howard 的问题**:把范围从"只用 Underground"扩大到"全部
NUMBAT 轨道站点"之后,单纯基于 BIC 的最优聚类数确实会变(从 6 变为
5),说明当前 K=5 的选择是以"仅 Underground"为前提的,
并非在任何站点范围下都成立的绝对结论。

## 发现二:原270站在新聚类中的簇归属稳定性

把 all-modes(403站)K=5 的结果限制回原来的270个 Underground 站点,与
...
## 发现三:新增的133个非 Underground 站点去了哪里
...
- K 选择本身对"是否纳入全部轨道模式"敏感,BIC 最优 K 从 6 变为 5,差异是真实的、不是网格误差。
- 但原270个 Underground 站点在新聚类中的簇归属部分一致
  (ARI=0.630),说明新增的133站主要是在"补充"聚类结构,
  而不是从根本上重新洗牌 Underground 站点原有的分组。
- 因此,"只用 Underground"这一决定对**270站内部的分组结论**总体上是稳健的,但如果要断言"K=5 是轨道交通夜间活动的普遍最优分类数",这个断言需要限定在"仅 Underground"这个范围内——扩大到全部 NUMBAT 轨道模式后,同样的方法学会给出不同的 K。这是一个关于**站点范围选择**的方法论说明,不是对现有 Underground 结果正确性的否定。

## 局限

- all-modes 的403站里有133个站点是本次检验才第一次纳入特征构建流程,没有
  经过与 canonical 270站同等程度的人工核查,尽管14处同址跨模式站点已在
