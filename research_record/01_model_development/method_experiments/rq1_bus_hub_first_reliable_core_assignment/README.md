...
Once sparse observations were recognised as a reliability problem, the next question was how to draw a threshold without choosing the desired cluster solution in advance. This workspace records that search. It moves from an initial fixed-K gate to a threshold-specific BIC screen, then compares pragmatic and literature-informed low-flow rules. The fact that some gates failed is part of the result: it prevented a provisional hub-first branch from being mistaken for the final Bus typology.

This is a side-by-side experiment. It does not overwrite the existing
