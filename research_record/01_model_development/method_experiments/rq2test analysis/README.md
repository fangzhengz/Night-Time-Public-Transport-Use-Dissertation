...
This was the project's first attempt to connect the emerging transport typologies to London's night-work geography. At that stage both the cluster solutions and the Rail catchment definition were still provisional, so the analysis is best read as a baseline: it established the compositional logic, exposed the importance of the spatial aggregation rule, and made clear which parts would need to be rerun once RQ1 stabilised.
