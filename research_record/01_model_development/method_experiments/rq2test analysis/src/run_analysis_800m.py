"""Run the RQ2 LNWC baseline analysis with an 800 m rail catchment radius.

Non-destructive sidecar to run_analysis.py: identical logic, only the
imported config module differs (config -> config_800m, which sets
RAIL_CATCHMENT_METRES=800 and redirects all outputs to
``rq2test analysis/outputs_800m``). The 1200 m canonical outputs under
``outputs/`` are never read or written by this script.

This directly answers item 2 of the original outputs/report/RESULTS_SUMMARY.md
"Interpretation limits and next validation gate" list ("Repeat rail analysis
with 800 m Voronoi and station-containing LSOA definitions."), first run
2026-07-24.
"""
...
def build_rail_analysis(lnwc: pd.DataFrame, lsoa: gpd.GeoDataFrame):
    ...
    catchments.to_crs("EPSG:4326").to_file(
        SPATIAL_OUT / "rail_catchments_800m.geojson", driver="GeoJSON"
    )
    ...
    axes[0].set_title(f"Rail RQ1 clusters and 800 m Voronoi catchments (K={RAIL_K})")
...
def main():
    ...
    lines = [
        ...
        "(sensitivity variant of the 1200 m canonical run in outputs/report/RESULTS_SUMMARY.md).",
        f"- Bus: provisional K={BUS_K}; direct LSOA-to-LNWC join (catchment-independent, identical to canonical).",
    ...
    lines.extend(
        [
            ...
            "2. This IS the 800 m Voronoi repeat requested by the canonical run's own validation "
            "gate item 2 -- compare directly against outputs/report/RESULTS_SUMMARY.md (1200 m).",
    ...
    run_metadata = {
        ...
        "command": "py -3 src/run_analysis_800m.py",
        "python": sys.version,
