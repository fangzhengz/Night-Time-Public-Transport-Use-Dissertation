"""IMD 2025 x RQ1 outputs -- provisional RQ2 extension (IMD2025 refresh).

Non-destructive parallel to run_imd_analysis.py: same two-line design, same
RQ1 shape-cluster typology, same continuous-metric definitions. The only
change is the deprivation source, from IMD 2019 (2011 LSOA, ONS best-fit
crosswalked to 2021) to IMD 2025 (natively published on 2021 LSOA
boundaries, MHCLG, released 2025-10). Outputs from run_imd_analysis.py are
left untouched under outputs/imd_metrics/; this script writes to
outputs/imd_metrics_2025/.
...
IMD 2025 (File 7, "All ranks, scores, deciles and population denominators")
is published directly on 2021 LSOA boundaries -- the same vintage as this
project's spatial units (LNWC, RQ1 bus LSOA clustering, LSOA boundary
geojson). No ONS best-fit crosswalk is needed: all 4,994 Greater London
2021 LSOAs matched directly (0 missing), versus 4,813/4,994 (96.4%) under
the IMD2019 crosswalk, which excluded 181 newly-split 2021 LSOAs with no
2011 source. See src/build_imd2025_lookup.py for the reformatting step
(IMD2025 File 7 -> outputs/../IMDdata_2025/imd2025_lsoa21_london.csv).
...
"add cluster as a covariate" as the fix, it was proposed and rejected. This
status is about the pooled test design, not the IMD vintage, so it is
unchanged by the IMD2025 refresh.
...
OUT = ROOT / "outputs" / "imd_metrics_2025"
DATA = OUT / "data"
FIGURES = OUT / "figures"
REPORT = OUT / "report"
...
def plot_cluster_imd(bus: pd.DataFrame, bus_weak: pd.DataFrame, rail: pd.DataFrame, rail_weak: pd.DataFrame) -> None:
    """Weak-line figure: RQ1 cluster vs IMD score distribution (bus + rail).

    This is the kept, primary weak-line result (cluster x IMD score,
    Kruskal-Wallis) -- distinct from the provisional main-line decile plot
    in plot_bus(). No figure existed for this result before; cluster labels
    stay numeric (C0, C1, ...) per project convention -- semantic naming
    belongs in the write-up, not in code/figures.
    """
    ...
    fig.suptitle("Weak line -- RQ1 shape cluster vs IMD 2025 score (kept, primary result)", y=1.02)
    plt.tight_layout()
...
def main() -> None:
    ...
    metadata = {
        ...
        "command": "py -3 src/run_imd2025_analysis.py",
        "python": sys.version,
    ...
    print(f"Completed IMD2025 extension in {metadata['duration_seconds']:.1f}s; bus={len(bus)}, rail={len(rail)}.")
