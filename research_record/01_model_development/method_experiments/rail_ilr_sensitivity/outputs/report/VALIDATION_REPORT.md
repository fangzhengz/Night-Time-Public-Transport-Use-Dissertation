...
- Origin Date: 2026-07-21T15:57:51.050445+00:00
...
## Transformation-driver diagnostic

The strong raw-versus-ILR disagreement triggered a **post-hoc mechanism audit**.
It is not used to select K. Full results are in `TRANSFORM_DIAGNOSTIC.md`.

- Zero-bin-count eta-squared at K=5 is `0.177` for the raw labels and `0.942`
  for the ILR labels.
- Spearman correlation between station distance and zero-pattern difference is
  `0.111` in raw space and `0.885` in ILR space.
- Zero-bin count and total activity have Spearman correlation `-0.429`.

The alpha=1 ILR result is therefore primarily stratified by temporal sparsity
and zero-pattern structure. It validly shows that raw K=5 fails this exact
sensitivity, but it does not by itself invalidate raw K=5 substantively or
justify promoting the internally more recurrent ILR K=4 to the primary typology.
...
5. Any future ILR primary model needs a prespecified zero/reliability sensitivity
   and a justified response to the coordinate dependence of diagonal GMMs.
...
The complete experiment was re-run deterministically under the independent
`outputs_repro` root. All 36 compared artifact groups matched exactly, including
features, K=2-12 labels, diagnostics, 200 bootstraps, 20 seed refits,
transformation-driver results, and nine figures. Status is therefore `VERIFIED`;
see `REPRODUCIBILITY_CHECK.md`.

Both runs emitted a NumPy `log(0)` warning while saving posterior entropy. All
saved entropy values are finite, and the warning occurs after fitting, so it
does not affect any clustering or validation result.
