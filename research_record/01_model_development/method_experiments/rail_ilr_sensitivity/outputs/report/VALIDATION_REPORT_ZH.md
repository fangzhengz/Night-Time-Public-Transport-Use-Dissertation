## Material Passport

- Origin Skill: experiment-agent
- Origin Mode: run + validate
- Origin Date: 2026-07-21T15:57:51.050445+00:00
- Verification Status: VERIFIED
- Version Label: rail_ilr_sensitivity_v1_zh

# Rail ILR 组成数据敏感性检验

## 核心结论

**当前 Rail K=5 对 ILR 组成数据处理存在实质敏感性，在进入下游解释前不应直接冻结标签。**

这是一项有边界的敏感性判断，并不意味着 K=5 是唯一真实或自然存在的站点类型数。

## 坐标与样本核验

- 样本保持为同一批 270 个 Underground 站点；
- 原始特征仍对应 344 个全周比例变量，entry 与 exit 各 172 个；
- 两个方向分别使用经验先验 `alpha=1` 处理零值，再各转为 171 个标准 Helmert ILR 坐标；
- 最终拟合维度为 342，中心化样本秩为 `269`；
- CLR/Aitchison 距离与 ILR 欧氏距离的最大抽样误差为 `2.842e-14`；
- 原始零值单元格共 `6989` 个，处理后最小比例为 `2.213e-10`。

## 原始比例标签与 ILR 标签的直接一致性

| raw_K | ilr_K | ARI   | NMI   | best_match_share | weakest_matched_cluster_jaccard |
| ----- | ----- | ----- | ----- | ---------------- | ------------------------------- |
| 5     | 5     | 0.216 | 0.351 | 0.511            | 0.207                           |
| 6     | 6     | 0.221 | 0.360 | 0.415            | 0.000                           |

### raw K=5 与 ILR K=5 的最佳簇匹配

| raw_cluster | ilr_cluster | intersection | reference_size | candidate_size | jaccard | recall | precision |
| ----------- | ----------- | ------------ | -------------- | -------------- | ------- | ------ | --------- |
| 0           | 4           | 46           | 119            | 53             | 0.365   | 0.387  | 0.868     |
| 1           | 2           | 12           | 15             | 39             | 0.286   | 0.800  | 0.308     |
| 2           | 0           | 20           | 44             | 52             | 0.263   | 0.455  | 0.385     |
| 3           | 3           | 17           | 38             | 61             | 0.207   | 0.447  | 0.279     |
| 4           | 1           | 43           | 54             | 65             | 0.566   | 0.796  | 0.662     |

强稳健性的预设标准要求 ARI、NMI、最佳匹配比例分别至少为 0.80、0.80、0.85，
且最弱匹配簇 Jaccard 不低于 0.50。本次标签一致性判定为：
`未达到强稳健标准`。

## ILR 内部诊断：固定 diagonal 协方差

| K | BIC       | delta_BIC_within_diag | silhouette | davies_bouldin | min_cluster_n | bootstrap_ARI_mean | bootstrap_ARI_q025 | bootstrap_ARI_q975 | bootstrap_weakest_cluster_jaccard | seed_ARI_mean |
| - | --------- | --------------------- | ---------- | -------------- | ------------- | ------------------ | ------------------ | ------------------ | --------------------------------- | ------------- |
| 3 | 76238.151 | 18734.701             | 0.322      | 2.165          | 52            | 0.723              | 0.489              | 0.992              | 0.443                             | 1.000         |
| 4 | 67853.199 | 10349.749             | 0.126      | 2.619          | 52            | 0.753              | 0.525              | 0.953              | 0.722                             | 1.000         |
| 5 | 63478.030 | 5974.580              | 0.082      | 2.862          | 39            | 0.559              | 0.371              | 0.747              | 0.091                             | 0.544         |
| 6 | 58633.800 | 1130.350              | 0.091      | 2.548          | 30            | 0.607              | 0.441              | 0.766              | 0.175                             | 0.808         |
| 7 | 59373.453 | 1870.003              | 0.060      | 2.685          | 25            | 0.563              | 0.433              | 0.705              | 0.310                             | 0.722         |
| 8 | 58580.577 | 1077.127              | 0.063      | 2.463          | 2             | 0.526              | 0.432              | 0.682              | 0.122                             | 0.718         |

- ILR diagonal 家族内 BIC 最低的是 K=9；
- K=3-8 范围内 silhouette 最高的是 K=3；
- K=3-8 范围内 bootstrap 平均 ARI 最高的是 K=4。

这些指标分别回答模型拟合、几何分离和重抽样复现问题，不能合并解释为发现了唯一真实 K。

## ILR K=5 的逐簇 bootstrap 复现性

| reference_cluster | jaccard_mean | jaccard_q025 | jaccard_q975 | share_jaccard_below_0_50 |
| ----------------- | ------------ | ------------ | ------------ | ------------------------ |
| 0                 | 0.091        | 0.000        | 0.619        | 0.935                    |
| 1                 | 0.665        | 0.405        | 0.929        | 0.240                    |
| 2                 | 0.652        | 0.025        | 0.857        | 0.045                    |
| 3                 | 0.624        | 0.410        | 0.938        | 0.265                    |
| 4                 | 0.558        | 0.000        | 0.761        | 0.185                    |

ILR K=5 的全局 bootstrap 平均 ARI 为 `0.559`，
95%经验区间为 `[0.371, 0.747]`；
最弱簇平均 Jaccard 为 `0.091`。按照预设规则，内部复现性
`未达到可接受标准`。

## 四类协方差 BIC 网格（次级诊断）

| covariance | best_K | best_BIC   | min_cluster_n | parameters_per_station | full_covariance_underidentified |
| ---------- | ------ | ---------- | ------------- | ---------------------- | ------------------------------- |
| spherical  | 12     | 176877.613 | 3             | 15.285                 | False                           |
| diag       | 9      | 57503.450  | 1             | 22.830                 | False                           |
| tied       | 12     | -79084.573 | 3             | 232.474                | False                           |
| full       | 2      | -23440.146 | 126           | 437.004                | True                            |

diagonal 仍作为主检验，是因为它固定了当前 Rail 分析的协方差假设。342 维特征下，
如果 full-covariance 成分的成员数不超过特征维度，其协方差在结构上无法由该成分样本
充分识别；即使正则化使模型数值上可拟合，也不应仅凭其 BIC 替换主检验。

## 转换驱动诊断

由于 raw 与 ILR 标签出现强烈不一致，另做了一项**事后机制审计**；它不参与 K 的选择，
只用于判断差异主要来自哪里。完整结果见 `TRANSFORM_DIAGNOSTIC.md`。

- K=5 的零时间格数量 η²：raw 标签为 `0.177`，ILR 标签为 `0.942`；
- 站点距离与零值模式差异的 Spearman ρ：raw 空间为 `0.111`，ILR 空间为 `0.885`；
- 零时间格数量与总活动量的 Spearman ρ 为 `-0.429`。

因此，本次 `alpha=1` ILR 结果主要按时间稀疏度/零值模式分层。它有效地说明 raw K=5
无法通过这项特定敏感性检验，但不能单凭这一结果断言 raw K=5 在实质上无效，也不能
把内部复现性较高的 ILR K=4 直接提升为新的站点类型主方案。

## 对论文论述的边界

1. 不比较 raw-share 344 维拟合与 ILR 342 维拟合的绝对 BIC。
2. ILR 改变了比例数据的几何表达和零值处理，但没有加入新的乘客信息。
3. LNWC、IMD、地理位置、客流规模及服务供给均未参与模型选择。
4. 标签一致性与 bootstrap 稳定性不能自动赋予簇以功能、社会经济或因果含义。
5. 若未来要把 ILR 作为主模型，必须进一步预设零值/可靠性敏感性，并说明 diagonal
   GMM 对所选 ILR 基底方向的依赖，或采用有充分依据的坐标不变模型策略。

## 统计谬误扫描

- 覆盖：11/11。
- 通过预先固定 K 范围、主协方差、判据和完整披露 K=3-8，降低了 look-elsewhere 与 garden of forking paths 风险。
- 未进行面积到个体的推断，因此没有使用生态谬误式解释。
- 本检验不提出因果或反向因果结论。
- Simpson's paradox、Berkson's paradox、collider bias、base-rate neglect、
  regression to the mean 与 survivorship bias 未被本聚类敏感性设计触发。

## 可复现性状态

完整实验已在独立的 `outputs_repro` 目录中确定性复跑。342 维特征、K=2–12 标签、
BIC（排除耗时列）、内部指标、200 次 bootstrap、20 次 seed refit、raw–ILR 比较、
转换驱动诊断以及 9 张图在 36 组比较中全部精确一致。因此本报告状态为 `VERIFIED`；
详细记录见 `REPRODUCIBILITY_CHECK.md`。

两次运行均在 posterior entropy 保存阶段出现 NumPy `log(0)` 运行时警告，但所有 entropy
值均为有限数，且警告发生在模型拟合之后，不影响特征、标签或任何结论指标。
