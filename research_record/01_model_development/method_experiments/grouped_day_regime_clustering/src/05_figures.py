# -*- coding: utf-8 -*-
"""05 · Figures (GROUPED) — per (modality, day-group) dataset, uniform style.

For each of rail_weekday / rail_weekend / bus_weekday / bus_weekend:
  - BIC grid (BIC vs K, one line per covariance);
  - K-diagnostics 2x3 panel (silhouette, CH, DB, BIC, bootstrap ARI);
  - per candidate K: cluster profiles (rows = clusters, both directions over the
    group's bins) and a spatial map (rail = station points, bus = LSOA choropleth).
"""
import sys
...
def parse_cols(cols):
    recs = []
    for c in cols:
        direction, mins = c.rsplit("_", 1)
        recs.append((c, direction, int(mins)))
...
def plot_bic_grid(ds):
    ...
    ax.set_title(f"{ds} — GMM BIC by covariance × K")
...
def plot_kdiag(ds):
    ...
    fig.suptitle(f"{ds} — K-diagnostics (BtC raw-share, GMM)", fontsize=14, y=1.0)
    fig.tight_layout(); fig.savefig(C.FIG / f"{ds}_kdiag.png", dpi=150, bbox_inches="tight"); plt.close(fig)


def plot_profiles(ds, X, cmap, lab, dirs, K):
    cls = sorted(pd.unique(lab))
    fig, axs = plt.subplots(len(cls), 1, figsize=(8, max(1.7 * len(cls), 4)), squeeze=False, sharey=True)
    for a, cl in zip(axs[:, 0], cls):
        m = lab == cl
        for direction, color in zip(dirs, [C.GREEN, C.RED]):
            sel = cmap[cmap.direction == direction].sort_values("minute")
            t = sel.minute.values
        ...
        a.set_xticks(tk); a.set_xticklabels([tlabel(x) for x in tk], fontsize=8)
        a.grid(axis="y", color="#eee", lw=.5); a.spines[["top", "right"]].set_visible(False)
    axs[0, 0].legend(fontsize=8, loc="upper right")
    fig.suptitle(f"{ds} K={K} cluster profiles (share over the night)", y=1.0)
...
def rail_map(ds, lab_df, K, base):
    coords = pd.read_csv(C.PRE / "rail_coords.csv", dtype={"NLC": str}).set_index("NLC")
    ...
    ax.set_title(f"{ds} K={K} — station clusters"); ax.legend(loc="lower right", fontsize=9); ax.set_axis_off()
...
def bus_map(ds, lab_df, K, base, code_col):
    ...
    ax.set_title(f"{ds} K={K} — LSOA clusters"); ax.legend(handles=handles, loc="lower right", fontsize=9)
    ax.set_axis_off()
    fig.tight_layout(); fig.savefig(C.FIG / f"{ds}_k{K}_map.png", dpi=150, bbox_inches="tight"); plt.close(fig)
...
def main():
    ...
    datasets = ([("rail", g) for g in C.RAIL_GROUPS] + [("bus", g) for g in C.BUS_GROUPS])
    for mod, grp in datasets:
        ds = f"{mod}_{grp}"
        ...
        for K in C.CAND_K:
            ...
            plot_profiles(ds, X, cmap, lab, dirs, K)
            if mod == "rail":
