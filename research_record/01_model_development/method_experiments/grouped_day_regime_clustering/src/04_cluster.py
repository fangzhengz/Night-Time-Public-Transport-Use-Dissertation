# -*- coding: utf-8 -*-
"""04 · GMM clustering + diagnostics — per (modality, day-group) dataset.

Four datasets: rail_weekday, rail_weekend, bus_weekday, bus_weekend.
For each:
  1. BIC grid over covariance type x K  -> choose (cov, K) minimising BIC.
  2. Using the BIC-best covariance, sweep K: silhouette / CH / DB + bootstrap ARI.
  3. For each candidate K, fit GMM and write labels (+ max_posterior, entropy).

Outputs (per dataset {ds}):
  diagnostics/{ds}_bic_grid.csv, {ds}_kdiag.csv, {ds}_bic_best.txt
  labels/{ds}_k{K}_labels.csv
"""
import sys
...
def main():
    for mod in ["rail", "bus"]:
        groups = C.RAIL_GROUPS if mod == "rail" else C.BUS_GROUPS
        for grp in groups:
            run(f"{mod}_{grp}", C.FEAT / f"X_{mod}_{grp}.parquet")
    print("done")
