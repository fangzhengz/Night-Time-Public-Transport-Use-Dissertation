...
The same hourly counts can tell a different story depending on where a composition is made to sum to one. This side study revisited an assumption that had quietly entered the Bus workflow: whether each direction should close across the full week or separately within each day type. By changing that denominator while holding the source observations fixed, it shows which apparent behavioural distinctions depend on the normalisation itself.

This is a sidecar test of the **normalisation denominator** on the bus leg. Read-only:
