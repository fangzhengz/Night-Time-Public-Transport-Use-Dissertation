...
Several early Bus revisions changed spatial aggregation, filtering and smoothing at the same time, making it impossible to know which decision caused the resulting cluster shift. This experiment was created to recover that missing counterfactual. It changes the stop-to-LSOA rule alone and leaves the rest of the historical pipeline intact, so any difference can be attributed to the spatial-unit construction rather than to a bundle of later corrections.

The single-variable test asks whether changing ONLY the stop-to-LSOA aggregation method
(hub-first vs the original point-in-polygon assignment) changes the bus
