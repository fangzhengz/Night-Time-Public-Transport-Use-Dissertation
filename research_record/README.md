...
|---|---|---|
| `00_early_pipeline_history/` | Original preprocessing, KMeans/CLARA/GMM and feature-v2 scripts from the exploratory stage | A compact historical code layer; duplicated early output trees are indexed rather than copied |
...
| `02_context_alternatives/` | The 1,200 m to 800 m LNWC development path, independent-variable development, the earlier direct-catchment facility implementation, LOAC, Spatial Signatures and earlier RQ2 implementations | Separates final-method ancestry and spatial sensitivities from context side analyses; the final POI variables use the LSOA-first 20-variable pipeline rather than the archived direct-catchment facility version |
| `03_incomplete_mobility_mismatch_analysis/` | Attempted comparison of area-level mobility/OD activity and observed public-transport use | Not completed or implemented as a formal dissertation question; preserved code and stale historical outputs require the warnings in its `STATUS.md` |
...
`STATUS.csv` is the study-level index and `SOURCE_COVERAGE.csv` links the local source branches to their curated destinations or explicit exclusions. Read them before opening an output. The retained READMEs, code, tables, reports and figures show what was actually attempted, including negative findings and explicitly unexecuted stages.
