...
results -- area-weighted LSOA-aggregation sensitivity sidecar.

CHANGED MEANING 2026-08-08: this script's own composition logic (area-weighted
by piece_area_m2, i.e. the ORIGINAL design) was deliberately left unchanged
here while run_lnwc_analysis.py was switched to equal-weight LSOA aggregation
at the same 800 m radius (config_800m.py's radius no longer differs from
config.py's). This script is therefore now the area-weighting sensitivity
check against run_lnwc_analysis.py's equal-weight primary result, not a radius
check. Outputs still land under outputs_800m/ so neither run overwrites the
other.
"""
...
def build_bus_analysis(lnwc: pd.DataFrame, lsoa: gpd.GeoDataFrame):
    ...
    # Named legend + fixed palette, matching the 400m script's 2026-08-08 fix
    # (run_lnwc_analysis.py) -- this 800m variant still plotted the bare
    # "cluster" column under cmap="tab10", so its colours never matched the
    # official RQ1 bus cluster maps (map_style.py's CLUSTER_COLOURS) and its
    # legend showed raw ids (0.0/1.0/2.0/3.0) instead of cluster names.
...
def build_rail_analysis(lnwc: pd.DataFrame, lsoa: gpd.GeoDataFrame):
    ...
    intersections["area_share"] = intersections["piece_area_m2"] / intersections["covered_area_m2"]

    shares = (
        intersections.pivot_table(index="NLC", columns="lnc_grp", values="area_share", aggfunc="sum", fill_value=0)
    ...
    # Unified 2026-08-14 (user decision): matches run_lnwc_analysis.py -- see
    # that file's docstring for the reasoning.
    ...
    catchments.to_crs(C.CRS_WGS84).to_file(C.SPATIAL_OUT / "rail_catchments_800m_allmodes.geojson", driver="GeoJSON")
    ...
    catchment_map.plot(column="cluster", categorical=True, cmap="tab10", linewidth=0.2, edgecolor="white", ax=axes[0], legend=True)
    stations.plot(column="cluster", categorical=True, cmap="tab10", markersize=7, ax=axes[0])
    axes[0].set_title(f"Rail sensitivity clusters + 800 m Voronoi catchments (K={C.RAIL_K}, all-modes, NaPTAN-matched)")
...
def main():
    ...
    if missing:
        raise FileNotFoundError(
            f"Missing required inputs (run run_context_metrics_800m.py first): {missing}"
        )
    ...
    print(f"Building 800 m rail Voronoi-clipped catchments over the {rail_n}-station clustering-eligible all-modes refit...")
    ...
    lines = [
        ...
        "## Scope",
        "",
        f"- Rail: all-modes merged sensitivity clustering K={C.RAIL_K}; {C.RAIL_CATCHMENT_METRES} m Voronoi-clipped catchments rebuilt over all {rail_audit['input_rows']} stations (sensitivity variant of this folder's own 1200 m run in outputs/report/LNWC_ASSOCIATION.md).",
        f"- Bus: StopArea CLR sensitivity clustering K={C.BUS_K}; direct LSOA-to-LNWC join (catchment-independent, identical to the 1200 m run).",
        ...
        f"{rail_audit['stations_outside_lnwc_extent']} station points outside the LNWC extent.",
    ...
    metadata = {
        ...
        "command": "py -3 src/run_lnwc_analysis_800m.py", "python": sys.version, "platform": platform.platform(),
