"""Continuous variable layer + internal cluster-coherence test, for the two
new sensitivity clustering results (rail all-modes K=5, bus StopArea CLR
K=4).

This merges what the canonical pipeline keeps in two separate folders:
- ``rq1_context_metrics_analysis/src/run_rq1_context_analysis.py``
  (per-unit continuous metrics, cluster summaries, robust signatures,
  dashboards)
- ``rq1_context_metrics_analysis/src/run_cluster_metric_significance.py``
  (metric ~ cluster Kruskal-Wallis + epsilon-squared, i.e. does the cluster
  label explain variance in its own continuous profile)
...
carries them (built by ``rq1_bus_stoparea_clustering/src/01_prepare_features.py``);
...
# match numbat_all_area_test/src/02_build_features_allmodes.py after the same
...
def build_rail_metrics() -> tuple[pd.DataFrame, float]:
    ...
    # numbat_all_area_test/src/02_build_features_allmodes.py using the
