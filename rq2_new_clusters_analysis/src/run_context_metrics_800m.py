"""Continuous variable layer + internal cluster-coherence test -- 800 m
rail catchment sensitivity sidecar.

Identical to run_context_metrics.py in every respect (this stage is
catchment-independent -- no Voronoi/buffer geometry involved at all) except
it imports config_800m and therefore writes to outputs_800m/ instead of
outputs/. Rerun here (rather than copied from the 1200 m outputs) purely so
outputs_800m/ is a fully self-contained, independently reproducible pipeline
run with its own provenance metadata; the numeric results are expected to be
identical to the 1200 m run's bus/rail_unit_metrics.csv.
"""
...
# 2026-08-01: common 18:00-05:00 window, matching run_context_metrics.py and
# numbat_all_area_test/src/02_build_features_allmodes.py.
...
def build_rail_metrics() -> tuple[pd.DataFrame, float]:
    ...
    # See run_context_metrics.py: FRI/SAT-only on purpose (Night Tube), with
    # post_0100_share added alongside as the all-day-type counterpart.
...
def main() -> None:
    ...
    lines = [
        "# RQ2 sensitivity clusters -- continuous variable layer + internal coherence (800 m catchment sidecar)",
        "",
        "## Material Passport",
        "",
        f"- Origin Date: {generated}",
        "- Verification Status: ANALYZED",
        "- Version Label: rq2_new_clusters_context_800m_v1",
        "",
        "## Scope",
        "",
        f"- Rail: all-modes merged sensitivity clustering, K={C.RAIL_K} ({len(rail)} stations, vs canonical's 270 Underground-only).",
        f"- Bus: StopArea CLR sensitivity clustering, K={C.BUS_K} ({len(bus)} LSOAs, vs canonical's 3,372 at min36).",
        "- This stage is catchment-radius-independent (no spatial buffer involved); results are expected to be numerically identical to the 1200 m run's outputs/data/{rail,bus}_unit_metrics.csv. Rerun here only so outputs_800m/ is self-contained.",
        "",
    ...
    metadata = {
        ...
        "command": "py -3 src/run_context_metrics_800m.py",
        "python": sys.version,
