...
FYP_ROOT = SCRIPT_PATH.parents[1]
DEFAULT_INPUT_DIR = FYP_ROOT
DEFAULT_OUTPUT_DIR = FYP_ROOT / "outputs" / "preprocessed_busto"
...
NIGHT_BLOCK_BINS = [18 * 60, 21 * 60, 24 * 60, 27 * 60, 30 * 60]
NIGHT_BLOCK_LABELS = [
    "18:00-21:00",
    "21:00-00:00",
    "00:00-03:00",
...
# To build the 18:00-06:00 night window we must borrow each day-type's
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=30 * 60,
        help="Night-window end in extended minutes; default 06:00 next day. "
             "TfL's official traffic-day boundary (05:00, matching NUMBAT) was "
             "tested and rejected: it discards the 05:00-06:00 hour, which "
             "carries most of the discriminating signal between night-economy "
             "and ordinary LSOAs (mean post_midnight_share 0.105->0.062, "
             "std 0.058->0.048) -- confirmed via a controlled test that varied "
             "only the cutoff, holding the corrected day-type sourcing fixed.",
    )
    parser.add_argument(
        "--schema-only",
        action="store_true",
