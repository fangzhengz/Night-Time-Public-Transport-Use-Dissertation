...
6. [`research_record/`](research_record/)：与最终管路隔离的试验、敏感性、弃用 RQ3 和探索性分析记录；
7. [`paper/CASA0010_dissertation_FangzhengZhou.pdf`](paper/CASA0010_dissertation_FangzhengZhou.pdf)：原样保留的已提交论文。

GitHub 不分发受许可限制的原始 NUMBAT、BUSTO 和 OS Points of Interest 数据；仓库提供来源、校验边界和完整代码。分析代码按论文顺序位于 `analysis/01_data_preparation` 至 `analysis/05_reporting`。`python scripts/validate_repository.py` 可快速验证已提交结果，配置本地原始数据后可使用 `python scripts/run_pipeline.py --full` 完整重跑。

`research_record/` 中的内容不由正式管路执行，也不表示论文采用了相应结果。每项研究均保留其当时的样本、设定与局限，并通过状态索引区分已完成检验、负结果、被替代方案、未运行阶段及正式放弃的方向。
