"""RQ1 cluster x LNWC association, for the two new adopted clustering
results.
...
that the LSOA-level aggregation logic matches analysis/04_urban_context and
analysis/03_lnwc_context exactly. This is a different axis from the activity-weighted
...
def build_bus_analysis(lnwc: pd.DataFrame, lsoa: gpd.GeoDataFrame):
    ...
    axes[0].set_title(f"Bus adopted clusters (K={C.BUS_K}, StopArea CLR)")
    map_data.plot(
...
def build_rail_analysis(lnwc: pd.DataFrame, lsoa: gpd.GeoDataFrame):
    ...
    # project (see analysis/04_urban_context/src/01_build_variable_table.py).
    ...
    axes[0].set_title(f"Rail adopted clusters + {C.RAIL_CATCHMENT_METRES} m Voronoi catchments (K={C.RAIL_K}, all-modes, NaPTAN-matched)")
...
def main():
    ...
    lines = [
        "# Adopted clusters -- LNWC association",
        "",
        "## Material Passport",
        "",
        f"- Origin Date: {generated}",
        "- Verification Status: ANALYZED",
        "- Version Label: final_lnwc_v1",
        "",
        "## Scope",
        "",
        f"- Rail: all-modes merged adopted clustering K={C.RAIL_K}; {C.RAIL_CATCHMENT_METRES} m Voronoi-clipped catchments rebuilt over all {rail_audit['input_rows']} stations (not reused from the canonical 270-station geometry).",
        f"- Bus: StopArea CLR adopted clustering K={C.BUS_K}; direct LSOA-to-LNWC join.",
