...
def main() -> None:
    ...
    expected = {(cov, k) for cov in COVARIANCES for k in K_RANGE}
    if OUT_CSV.exists():
        cached = pd.read_csv(OUT_CSV)
        observed = set(zip(cached["covariance"], cached["K"].astype(int)))
    else:
        cached = pd.DataFrame()
        observed = set()
    if observed == expected:
        grid = cached
        print(f"Reusing complete computed grid: {OUT_CSV}")
    else:
        rows = []
