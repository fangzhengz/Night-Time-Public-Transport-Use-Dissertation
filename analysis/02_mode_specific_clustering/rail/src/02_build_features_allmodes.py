...
`FYP/analysis/02_mode_specific_clustering/rail/src/03_build_features.py` unchanged (that
...
separation from `analysis/02_mode_specific_clustering/bus/`). This script's own
...
`analysis/02_mode_specific_clustering/rail/src/config.py` so the resulting 344 columns
...
# Must match analysis/02_mode_specific_clustering/rail/src/config.py exactly.
...
#
# What it costs. Tested before adoption in FYP/docs/method_decisions.md/ (the
...
def assemble(per_day: dict, directions: list[str], days: list[str], keep_units=None):
    ...
    analysis/02_mode_specific_clustering/rail/src/03_build_features.py::assemble.
    """
