...
HOUR_TICK_LABELS = [f"{((h // 60) % 24):02d}" for h in C.HOURS]

FEATURE_FILES = {
    "raw_share": C.FEATURES / "X_bus_stoparea_raw_share_min33.parquet",
    "clr": C.FEATURES / "X_bus_stoparea_clr_min33.parquet",
}
...
def make_profiles_and_maps(
    ...
    for k in C.FIGURE_KS:
        ...
        fig.suptitle(
            f"StopArea allocation, min direction >={C.MIN_DIRECTION:g}, {variant}, K={k}\n"
    ...
    for k in C.FIGURE_KS:
        ...
        ax.set_title(
            f"StopArea allocation, {variant}, K={k}\n"
...
def run_variant(variant: str, bootstrap_n: int, n_init: int) -> None:
    ...
    ax.set(xlabel="K", ylabel="BIC", title=f"StopArea {variant}: BIC grid")
    ax.legend()
    ax.grid(alpha=0.25)
    fig.tight_layout()
    ...
    report = [
        ...
        f"- Version Label: stoparea_min33_{variant}_v1",
        "",
        f"# StopArea bus clustering: {variant}",
        "",
        "## Specification",
        "",
        f"- Sample: {len(X):,} LSOAs with total>=50 and both direction totals>={C.MIN_DIRECTION:g}.",
