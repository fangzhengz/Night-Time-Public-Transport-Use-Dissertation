...
def build_metrics(long: pd.DataFrame, grouped: pd.DataFrame) -> pd.DataFrame:
    ...
    # Single retention rule: both directions must clear MIN_DIRECTION over the
    # full week. The former `total_activity >= 50` companion rule was removed
    # It is implied by this one (both directions >= 33 gives total >= 66) and
    # never excludes an LSOA this rule does not already exclude.
    metrics["passes_min_direction_threshold"] = (
        metrics["min_direction_activity"] >= C.MIN_DIRECTION
    )
    metrics["retained_for_fit"] = metrics["passes_min_direction_threshold"]
    metrics["exclusion_reason"] = np.where(
        metrics["passes_min_direction_threshold"],
        "retained",
        "at_least_one_direction_below_33",
    )
    return metrics
...
def main() -> None:
    ...
    raw_counts.to_parquet(C.FEATURES / "raw_counts_min33.parquet")
    X_raw.to_parquet(C.FEATURES / "X_bus_stoparea_raw_share_min33.parquet")
    X_clr.to_parquet(C.FEATURES / "X_bus_stoparea_clr_min33.parquet")
    ...
    manifest_rows = [
        {
            "role": "canonical_stoparea_lsoa_long",
    ...
    if C.STOPAREA_SUMMARY.exists():
        manifest_rows.append(
            {
                "role": "stoparea_preprocessing_summary",
    ...
    report = [
        ...
        "",
        f"- Fixed: LSOA unit, three day types, 18:00-05:00 hourly window, "
        f"{len(C.HOURS) * len(C.DAY_TYPES)} bins per direction, min-direction threshold {C.MIN_DIRECTION:g}.",
