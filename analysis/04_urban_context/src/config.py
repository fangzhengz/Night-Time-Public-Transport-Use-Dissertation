...
RQ2 combines the seven-group LNWC classification with a set of specific
contextual variables. The project's own reference
...
Downloaded (ONS Census 2021, LSOA level):
  ...
  night_industry_share        -- TS060/TS061. Accommodation & food + human
                                 ...
                                 refit -- check analysis/03_lnwc_context/
...
Report it, but do not read a strong bus association as a standalone result: the bus
clustering is itself heavily activity-volume driven (activity epsilon² ~0.56),
and activity volume is near-mechanically related to density. Its job is to
...
#
# The split below uses two external, non-circular criteria:
...
#   2. The conceptual distinction between leisure-serving and shift-work
#      sectors, retained as contextual interpretation rather than an outcome.
...
RAIL_LABELS = FYP / "analysis" / "02_mode_specific_clustering" / "rail" / "outputs" / "data" / "rail_allmodes_k5_labels.csv"
RAIL_COORDS = FYP / "analysis" / "01_data_preparation" / "rail" / "outputs" / "data" / "rail_allmodes_coords.csv"
RAIL_UNIT_METRICS = FYP / "analysis" / "03_lnwc_context" / "outputs" / "data" / "rail_unit_metrics.csv"
RAIL_K = 5
EXPECTED_RAIL_UNITS = 403

BUS_LABELS = FYP / "analysis" / "02_mode_specific_clustering" / "bus" / "outputs_1805_min33" / "clr" / "labels" / "k4_labels.csv"
BUS_K = 4
...
# 800 m Voronoi-clipped station catchments, matching analysis/03_lnwc_context and
# analysis/03_lnwc_context's 800 m sidecar.
...
#
# analysis/02_mode_specific_clustering/rail/src/09_cluster_names.py now derives the names from the
...
def _load_rail_cluster_names(path=RAIL_CLUSTER_NAMES_FILE):
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"{path} is missing. Run analysis/02_mode_specific_clustering/rail/src/09_cluster_names.py; "
...
# Bus went through the same fix on 2026-08-02: analysis/02_mode_specific_clustering/bus/
...
# hardcoded copies (here, analysis/05_reporting, analysis/05_reporting)
...
def _load_bus_cluster_names(path=BUS_CLUSTER_NAMES_FILE):
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"{path} is missing. Run analysis/02_mode_specific_clustering/bus/src/"
...
# analysis/02_mode_specific_clustering/bus/src/config.py so figures here match the bus maps),
