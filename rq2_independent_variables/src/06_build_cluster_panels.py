...
behavioural metrics in rq2_new_clusters_analysis, and the area-context profile
...
BEHAVIOUR_METRICS = {
    "bus": ["log_total_activity", "post_2300_share", "deep_night_share",
            "post_midnight_persistence", "weekend_ratio", "direction_balance"],
    # Keep this list in sync with RAIL_METRICS in rq2_new_clusters_analysis.
    # Since 2026-08-06 both modes use the same 23:00 threshold while retaining
    # their own complete source windows (Rail to 05:00; Bus to 06:00).
...
METRIC_LABEL = {
    ...
    "deep_night_share": "deep-night share",
...
def profile_from_long(long: pd.DataFrame, unit: str, labels: pd.DataFrame) -> pd.DataFrame:
    ...
    (numbat_all_area_test/src/02_build_features_allmodes.py: "normalise each
...
def bus_profiles(labels: pd.DataFrame) -> pd.DataFrame:
    long = pd.read_parquet(
        C.FYP / "data_processing" / "bus_stoparea" / "outputs" / "preprocessed"
...
def rail_profiles(labels: pd.DataFrame) -> pd.DataFrame:
    long = pd.read_parquet(
        C.FYP / "data_processing" / "rail_allmodes" / "outputs" / "preprocessed"
    ...
    # (numbat_all_area_test/src/02_build_features_allmodes.py). Before that date
...
def build(mode: str) -> None:
    ...
    if mode == "bus":
        ...
        metrics_source = pd.read_csv(
            C.FYP / "rq2_new_clusters_analysis" / "outputs" / "data" / "bus_unit_metrics.csv"
        )
        ...
        metrics_source = pd.read_csv(
            C.FYP / "rq2_new_clusters_analysis" / "outputs" / "data" / "rail_unit_metrics.csv"
        )
