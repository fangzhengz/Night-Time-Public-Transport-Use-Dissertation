...
corrected across variables within each mode. Deliberately the same statistic
the LNWC/IMD/LOAC lenses already use, so the new variables can be read on the
same scale as the composite ones and dropped into the same comparison table.
...
Reading guide for the output:
  * population_density is a CONTROL. A strong bus association there is close to
    ...
    finding.
  * epsilon-squared is comparable to the LNWC/LOAC Cramer's V only loosely
    (different statistics, different bounds). Compare within this table, and
    against the existing IMD overall-score epsilon-squared -- printed live
    at the end of this script's run rather than typed here, since it drifts
    whenever the clustering it is computed on is refit (see
    config.load_composite_associations).
"""
...
def main() -> None:
    ...
    # Direct comparison against the composite lenses already in the project,
    # read live rather than typed here (see config.load_composite_associations).
    composites = C.load_composite_associations()
    print("\nFor reference, the composite lenses on the same clusterings:")
    for mode in ("bus", "rail"):
        v = composites[mode]
        print(f"  {mode:4}: LNWC Cramer's V {v['lnwc']:.3f} | LOAC {v['loac']:.3f} | "
              f"IMD overall epsilon2 {v['imd']:.3f}")
