...
`rq2_new_clusters_analysis`'s `run_lnwc_analysis.py`).
...
`numbat_all_area_test/src/02_build_features_allmodes.py`'s own
...
`numbat_all_area_test` validation report) already have a NaPTAN match --
...
def main() -> None:
    ...
    lines = [
        ...
        "clustering population** used by `numbat_all_area_test` -- a further "
        ...
        "polygon, as `rq2_new_clusters_analysis` does.",
    ]
